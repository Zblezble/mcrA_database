...
# ============================================================================
# 02a_download_genomes.sh
# Download completed genomes and WGS assemblies that contain mcrA genes.
#   1. Search NCBI nuccore with the mcrA genome query
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Searching NCBI nuccore for mcrA genome entries ..."
esearch -db nuccore -query "${QUERY_MCRA_GENOME}" | \
...
OUT_STORAGE="${STORAGE}/02a_genomes"
mkdir -p "${OUT_STORAGE}"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Step 02a complete. Results in ${OUT_STORAGE}"
